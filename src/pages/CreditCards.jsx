...
// *** UPDATED CreditCards Component ***
export const CreditCards = () => {
 const swiperAreaRef = useRef(null);
 const [swiperInstance, setSwiperInstance] = useState(null);
 
 // 1. Ref to manage the throttle timer
 ...
 // 2. THIS IS THE VALUE TO CONTROL SCROLL SPEED (in milliseconds)
 //  Increase for slower/less frequent scrolling, decrease for faster.
 const SCROLL_DELAY = 600;

 const handleWheel = (e) => {
 // If a scroll is already in progress and waiting for the delay, do nothing.
 ...
 // 3. Prioritize horizontal scroll (deltaX), fall back to vertical (deltaY)
 const scrollDelta = e.deltaX || e.deltaY;

 // Ensure there was significant scroll movement
 ...
 // 4. Set a timeout to prevent another scroll for the duration of SCROLL_DELAY
 ...
 // 5. Clear the timeout if the component unmounts to prevent memory leaks
 if (throttleTimeout.current) {
 ...
 // Removed the pagination prop here
 grabCursor={false}
 modules={[Pagination]}
...
// Unchanged FloatingCards Component
